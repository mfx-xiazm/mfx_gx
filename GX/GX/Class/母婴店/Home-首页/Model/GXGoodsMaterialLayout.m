...
@implementation GXGoodsMaterialLayout
...
- (void)resetLayout
{
    ...
    // 计算是否应该加上展示全文的高度
    if (_material.shouldShowMoreButton) { // 如果要显示全文按钮
        _height += kMomentLineSpacing;
        _height += kMomentHandleButtonHeight;
    }
    // 计算图片布局
    if (_material.photos.count != 0) {
        [self layoutPicture];
        _height += kMomentMarginPadding;
...
- (void)layoutText
{
    ...
    NSMutableAttributedString * text = [[NSMutableAttributedString alloc] initWithString:_material.dsp];
    ...
    YYTextContainer * container = [YYTextContainer containerWithSize:CGSizeMake(HX_SCREEN_WIDTH - 10*2, _material.isOpening ? CGFLOAT_MAX : 16 * lineCount + kMomentLineSpacing * (lineCount - 1))];
    // 阶段的类型，超出部分按尾部截段
...
- (void)layoutPicture
{
    ...
    self.photoContainerSize = [SDWeiXinPhotoContainerView getContainerSizeWithPicPathStringsArray:_material.photos];
}
@end
