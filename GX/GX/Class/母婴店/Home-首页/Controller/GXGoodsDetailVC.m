...
@interface GXGoodsDetailVC ()<UIScrollViewDelegate,UITableViewDelegate,UITableViewDataSource,GXGoodsMaterialCellDelegate,GXGoodsCommentCellDelegate,UICollectionViewDelegate,UICollectionViewDataSource,ZLCollectionViewBaseFlowLayoutDelegate,XQCarouselDelegate>
...
@property (weak, nonatomic) IBOutlet SPButton *control_customer_btn;
...
- (IBAction)customerBtnClicked:(UIButton *)sender {
    if (self.goodsDetail.provider_customer && self.goodsDetail.provider_customer.peerId.length) {
        ...
        wvc.url = [NSString stringWithFormat:@"https://ykf-webchat.7moor.com/wapchat.html?accessId=%@&fromUrl=%@&urlTitle=%@&language=ZHCN&otherParams={\"agent\":\"%@\",\"peerId\":\"%@\",\"nickName\":\"%@\",\"cardInfo\":{\"left\":{\"url\": \"%@\"},\"right1\": {\"text\": \"%@\",\"color\": \"#595959\",\"fontSize\": 12},\"right2\": {\"text\": \" \",\"color\": \"#595959\",\"fontSize\": 12},\"right3\": {\"text\": \"%@\",\"color\": \"#ff6b6b\",\"fontSize\": 14}}}&clientId=gxshop_%@&customField={\"userName\":\"%@\",\"userId\":\"%@\",\"userPhone\":\"%@\"}",
...
-(void)handleGoodsDetailData
{
    ...
    if (self.goodsDetail.provider_customer && self.goodsDetail.provider_customer.peerId.length) {
        ...
        self.control_customer_btn.hidden = NO;
    }else{
        self.normal_customer_btn.hidden = YES;
        self.control_customer_btn.hidden = YES;
    }
}
