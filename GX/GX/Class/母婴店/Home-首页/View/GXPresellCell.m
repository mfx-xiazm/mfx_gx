...
-(void)setPreSale:(GXPreSales *)preSale
{
    ...
    if ([_preSale.sell_status isEqualToString:@"1"]) {
        ...
        [self.handleLabel setText:@"即将开始"];
        [self.handleLabel setTextColor:UIColorFromRGB(0xFF8A00)];
        [self.handleLabel setBackgroundColor:UIColorFromRGB(0xEEEEEE)];
        ...
        [self.handleLabel setText:@"立即购买"];
        [self.handleLabel setTextColor:[UIColor whiteColor]];
        [self.handleLabel setBackgroundColor:UIColorFromRGB(0xFF8A00)];
    }else{
        self.flag_img.hidden = YES;
        self.sale_num.hidden = NO;
        self.sale_num.text = [NSString stringWithFormat:@"销量：%@",_preSale.sell_num];
        [self.handleLabel setText:@"已结束"];
        [self.handleLabel setTextColor:UIColorFromRGB(0xFF8A00)];
        [self.handleLabel setBackgroundColor:UIColorFromRGB(0xEEEEEE)];
    }
