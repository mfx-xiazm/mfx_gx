...
@required;
- (NSInteger)sp_numberOfComponentsInPickerView:(SPPickerView *_Nonnull)pickerView; // 返回多少列
- (NSInteger)sp_pickerView:(SPPickerView *_Nonnull)pickerView numberOfRowsInComponent:(NSInteger)component; // 每一列返回多少行
@end
...
- (CGFloat)sp_pickerView:(SPPickerView *_Nonnull)pickerView rowHeightForComponent:(NSInteger)component; // 返回每一列每一行的行高
- (CGFloat)sp_pickerView:(SPPickerView *_Nonnull)pickerView rowWidthForComponent:(NSInteger)component; // 返回每一列每一行的行宽

// 下面这3个代理方法，优先级逐次提高，即：如若以下3个代理方法同时实现，则会按照优先级较高的显示
- (nullable NSString *)sp_pickerView:(SPPickerView *_Nonnull)pickerView titleForRow:(NSInteger)row forComponent:(NSInteger)component; // 返回每一列每一行的普通文本
- (nullable NSAttributedString *)sp_pickerView:(SPPickerView *_Nonnull)pickerView attributedTitleForRow:(NSInteger)row forComponent:(NSInteger)component;  // 返回每一列每一行的富文本
- (UITableViewCell *_Nullable)sp_pickerView:(SPPickerView *_Nonnull)pickerView cellForRow:(NSInteger)row forComponent:(NSInteger)component systemCell:(UITableViewCell *_Nullable)systemCell; // 自定义某一列中的cell，如果不想自定义，可直接使用systemCell，对systemCell上的textLabel等控件设置内容

- (void)sp_pickerView:(SPPickerView *_Nonnull)pickerView didSelectRow:(NSInteger)row inComponent:(NSInteger)component; // 选中了哪一列哪一行
@end


@interface SPPickerView : UIView
@property (nonatomic, weak, nullable) id <SPPickerViewDatasource> dataSource;
@property (nonatomic, weak, nullable) id <SPPickerViewDelegate> delegate;
...
- (void)sp_replaceTableViewAtComponent:(NSInteger)component withTableView:(UITableView *_Nullable)tableView;
@end
