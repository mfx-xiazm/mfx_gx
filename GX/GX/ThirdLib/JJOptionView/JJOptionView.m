...
-(void)layoutSubviews
{
    ...
    self.rightImageView.hxn_x = self.hxn_width - 20.f;
    self.rightImageView.hxn_y = self.hxn_height/2.0 - 5.f;
    
    self.titleLabel.hxn_x = 10.f;
    self.titleLabel.hxn_y = 0.f;
    self.titleLabel.hxn_width = self.hxn_width - 40.f;
    self.titleLabel.hxn_height = self.hxn_height;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    cell.textLabel.textColor = UIColorFromRGB(0x131d2d);
...
- (UIImageView *)rightImageView {
    if(!_rightImageView) {
        _rightImageView = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"下"]];
...
- (UITableView *)tableView {
    if (!_tableView) {
        ...
        _tableView.layer.borderWidth = 0.f;
...
- (void)setDataSource:(NSArray *)dataSource {
    ...
    if (self.rowHeigt) {
        if (_dataSource.count>=6) {
            self.tableViewHeight = 6*self.rowHeigt;
        }else{
            self.tableViewHeight = self.dataSource.count*self.rowHeigt;
        }
    }else {
        if (_dataSource.count>=6) {
            self.tableViewHeight = 6*rowheight;
        }else{
