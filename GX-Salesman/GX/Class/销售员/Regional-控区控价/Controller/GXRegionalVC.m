        ...
        _header.regionalClickedCall = ^(NSInteger type, NSInteger index) {
            ...
            if (type == 1) {
                ...
                if (index == 1) {
                    ...
                    GXPartnerIncomeVC *dvc = [GXPartnerIncomeVC new];
                    [strongSelf.navigationController pushViewController:dvc animated:YES];
                }else{
...
-(void)getRegionalControlDataRequest
{
    ...
    dispatch_group_async(group, queue, ^{
        ...
        [HXNetworkTool POST:HXRC_M_URL action:@"program/controlData" parameters:@{} success:^(id responseObject) {
...
-(void)getHotBrandDataRequest:(BOOL)isRefresh completedCall:(void(^)(void))completedCall
{
    ...
    hx_weakify(self);
    [HXNetworkTool POST:HXRC_M_URL action:@"program/getHotBrand" parameters:parameters success:^(id responseObject) {
        hx_strongify(weakSelf);
        if([[responseObject objectForKey:@"status"] integerValue] == 1) {
            if (isRefresh) {
