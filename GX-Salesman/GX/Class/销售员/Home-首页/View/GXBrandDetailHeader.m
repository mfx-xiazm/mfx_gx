...
@property (weak, nonatomic) IBOutlet UILabel *reject_resion;
@property (weak, nonatomic) IBOutlet UILabel *join_desc;
...
-(void)setBrandDetail:(GXBrandDetail *)brandDetail
{
    ...
    [self.join_desc setTextWithLineSpace:5.f withString:(_brandDetail.brand_join_desc && _brandDetail.brand_join_desc.length)?[NSString stringWithFormat:@"加盟提示：%@",_brandDetail.brand_join_desc]:@"加盟提示：无" withFont:[UIFont systemFontOfSize:12]];
    self.join_desc.textAlignment = NSTextAlignmentCenter;
    ...
    if ([_brandDetail.apply_status isEqualToString:@"2"]) {// 合作中 没有加盟按钮
        ...
        self.reject_resion.text = @"";
        self.reject_resion.hidden = YES;
        ...
        if ([_brandDetail.apply_status isEqualToString:@"0"]) {//未加盟
            ...
            self.reject_resion.hidden = NO;
            [self.reject_resion setTextWithLineSpace:5.f withString:(_brandDetail.reject_reason && _brandDetail.reject_reason.length)?[NSString stringWithFormat:@"驳回原因：%@",_brandDetail.reject_reason]:@"" withFont:[UIFont systemFontOfSize:12]];
            self.reject_resion.textAlignment = NSTextAlignmentCenter;
