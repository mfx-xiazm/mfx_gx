...
-(void)getHomeDataRequest:(BOOL)isRefresh
{
    ...
    hx_weakify(self);
    [HXNetworkTool POST:HXRC_M_URL action:@"program/getHomeData" parameters:parameters success:^(id responseObject) {
        hx_strongify(weakSelf);
...
-(void)updateVersionRequest
{
    ...
    [HXNetworkTool POST:HXRC_M_URL action:@"program/isNewVersions" parameters:@{@"sys":@"2",@"versions":currentVersion} success:^(id responseObject) {
        hx_strongify(weakSelf);
