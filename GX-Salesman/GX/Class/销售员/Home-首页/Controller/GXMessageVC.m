...
-(void)getMessageDataRequest:(BOOL)isRefresh
{
    ...
    hx_weakify(self);
    [HXNetworkTool POST:HXRC_M_URL action:[[MSUserManager sharedInstance].curUserInfo.utype isEqualToString:@"3"]?@"program/getMessageData":@"program/getMessageData" parameters:parameters success:^(id responseObject) {
        hx_strongify(weakSelf);
...
-(void)readMsgRequest:(NSString *)msg_id
{
    ...
    [HXNetworkTool POST:HXRC_M_URL action:[[MSUserManager sharedInstance].curUserInfo.utype isEqualToString:@"3"]?@"program/readMsg":@"program/readMsg" parameters:parameters success:^(id responseObject) {
        if([[responseObject objectForKey:@"status"] integerValue] == 1) {
