...
-(void)getCurrencyQuotationsRequest
{
    ...
    [HXNetworkTool POST:HXRC_M_URL action:@"program/currencyQuotations" parameters:parameters success:^(id responseObject) {
...
-(void)addOrderCartRequest:(NSString *)goods_id specs_attrs:(NSString *)specs_attrs freight_template_id:(NSString *)freight_template_id sku_id:(NSString *)sku_id
{
    ...
    [HXNetworkTool POST:HXRC_M_URL action:@"program/addOrderCart" parameters:parameters success:^(id responseObject) {
        if([[responseObject objectForKey:@"status"] integerValue] == 1) {
            [MBProgressHUD showTitleToView:nil postion:NHHUDPostionCenten title:[responseObject objectForKey:@"message"]];
        }else{
            [MBProgressHUD showTitleToView:nil postion:NHHUDPostionCenten title:[responseObject objectForKey:@"message"]];
