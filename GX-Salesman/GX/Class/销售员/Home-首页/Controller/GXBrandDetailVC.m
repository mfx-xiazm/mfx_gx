...
-(void)getBrandDetailRequest
{
    ...
    dispatch_group_async(group, queue, ^{
        ...
        [HXNetworkTool POST:HXRC_M_URL action:@"program/brandDetail" parameters:parameters success:^(id responseObject) {
...
-(void)getGoodsListDataRequest:(BOOL)isRefresh completedCall:(void(^)(void))completedCall
{
    ...
    hx_weakify(self);
    [HXNetworkTool POST:HXRC_M_URL action:@"program/getBrandGoodList" parameters:parameters success:^(id responseObject) {
        hx_strongify(weakSelf);
        if([[responseObject objectForKey:@"status"] integerValue] == 1) {
            if (isRefresh) {
...
-(void)joinBrandRequest
{
    ...
    hx_weakify(self);
    [HXNetworkTool POST:HXRC_M_URL action:@"program/joinBrand" parameters:parameters success:^(id responseObject) {
        hx_strongify(weakSelf);
        if([[responseObject objectForKey:@"status"] integerValue] == 1) {
...
- (CGSize)collectionView:(UICollectionView *)collectionView layout:(UICollectionViewLayout *)collectionViewLayout referenceSizeForHeaderInSection:(NSInteger)section {
    ...
    return CGSizeMake(collectionView.frame.size.width, height+30.f);
}
